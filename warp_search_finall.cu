...
__global__
void warp_independent_search_kernel(value_t* d_data,value_t* d_query,idx_t* d_result,graph_node<dim,degree>* d_graph,\
...
static void astar_multi_start_search_batch(const std::vector<std::vector<std::pair<int,value_t>>>& queries,int k,\
	std::vector<std::vector<idx_t>>& results,std::vector<int>& step_num,value_t* h_data,graph_node<dim,degree>* h_graph,pq_value_t* pq_centroid,int num){
	value_t* d_data;
	value_t* d_query;
    idx_t* d_result;
...
int main() {
	...
	std::ifstream file("/home/xy/anns-2/ann_search/mini_graph/bfsg.data", std::ios::binary | std::ios::ate);
	if (!file.is_open()) {
    ...
    astar_multi_start_search_batch(queries, TOPK, results, step_num, h_data, h_graph, pq_centroid, num_vertices);
